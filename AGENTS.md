...
- Создание, удаление и очистку игровых объектов проводить через существующий request/server-named поток проекта; не создавать игровые объекты напрямую из UI-кнопок.
...
- не делай проверку через GIT и HEMTT
...
- `ptg_ui`: диалоги, стартовое меню, экран техники, режим водитель+стрелок, свободная камера, телепорт, перевооружение, тестовые цели, обработчики кнопок, HUD статуса объекта.
- `ptg_spawn`: request/server-named поток спавна, реестр созданных объектов, создание водителя/экипажа, создание/удаление тестовых целей.
...
- Для режима “Создать с экипажем” сохранять текущую схему: водитель создается классом игрока, игрок садится в слот стрелка, управление движением идет через штатные input actions Arma 3, без ручного `setDriveOnPath`/`setVelocityModelSpace`.
...
- Для спавн/delete/цели/rearm проверить, что действия с объектами выполняются на машине-владельце объекта или через существующий request/server-named поток проекта.
