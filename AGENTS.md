...
- Если изменение меняет пользовательское поведение, публичный workflow, архитектурную границу модуля или локализационные/UI-контракты, обновить профильный README модуля и/или релевантный документ из `docs/ptg/`. Для мелких bugfix/refactor без изменения поведения документацию не трогать.
